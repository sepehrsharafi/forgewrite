...
const FallingBricks = ({ height, isReady }: Props) => {
  ...
  const handlePointerDown = (
    ...
    if (isMobile) {
      // "Explode" the brick: give it a strong upward and random lateral velocity
      const angle = (Math.random() - 0.5) * Math.PI; // random angle left/right
      const speed = 70 + Math.random() * 1; // strong velocity
      const vx = Math.cos(angle) * speed;
      const vy = -Math.abs(Math.sin(angle) * speed) - 1200; // always up

      body.setLinvel({ x: vx, y: vy }, true);
      body.setAngvel((Math.random() - 0.5) * 12, true); // random spin
      body.wakeUp();
